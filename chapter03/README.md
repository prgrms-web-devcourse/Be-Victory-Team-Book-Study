...
- 객체의 자율성을 보장한다. (내부로직을 건드리지 않도록 하고. 우리의 객체의 메시지만 전달하면. )
- protected를 많이 쓰나요?
...
- protected
  ...
  - 이패키지에서 쓰는 변수명과 다른패키지에서 쓰는 이름이 같을 때, protected
- default
...
- 정적 멤버는 `클래스명.정적멤버` 형식으로 접근하는 것을 권장.
  - 왜? 일관된 형식으로 접근하기 위해.
  - `사람.인구수` vs `홍길동.인구수` => 이러면 안되니까 물리적 접근에 따른 이유
...
### Q. 지역 변수를 초기화 안하는 경우 어떤 일이?

A. 이상한 값이 들어가는게 아닌, 런타임이 아닌 컴파일 시점에 에러를 반환한다.

### Q. ISP & LSP

A. 높은 응집도, 낮은 결합도. LSP는 repository, jdbc, jparepository만든다고했을때 각각이 공통기능을 가지는게 아니자낭. 이걸 ISP, LSP대입해서 . 응집도 결합도 보면 되나?
